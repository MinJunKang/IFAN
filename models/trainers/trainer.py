...
class Model(baseModel):
    def __init__(self, config):
        ...
        if self.rank <= 0 : print('Loading Model...')
        ...
        if self.is_train:
            ...
            if self.rank <= 0: print('\tinitializing LPIPS')
            if config.dist:
        ...
        ### DDP ###
        if config.dist:
            if self.rank <= 0: print('Building Dist Parallel Model...')
    ...
    def _set_loss(self, lr = None):
        if self.rank <= 0: print('Building Loss...')
    ...
    def _set_optim(self, lr = None):
        if self.rank <= 0: print('Building Optim...')
        self._set_loss()
    ...
    def _set_dataloader(self):
        if self.rank <= 0: print('Loading Data Loader...')
...
class DeblurNet(nn.Module):
    def __init__(self, config, lib):
        ...
        if self.rank <= 0: print('\tinitializing deblurring network')

        if self.config.is_train and ('R' in config.mode or 'IFAN' in config.mode):
            if self.rank <= 0: print('\tinitializing RBN')
